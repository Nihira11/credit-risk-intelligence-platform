...
- **Data:** 307,511 applicants, 40 WOE-transformed features, 8% default rate
- **Target Metrics:** Logistic Gini ≥ 0.45, KS ≥ 0.25
- **Approach:** Stratified train/test split (70/30), honest validation (no leakage)
...
cat("Applicants: 307,511 | Features:", ncol(df_model) - 1, "\n\n")
...
```{r logistic_fit}
# create formula
feature_names <- setdiff(names(train), "target")
...
# sign distribution
n_pos <- sum(coef_df$estimate > 0)
n_neg <- sum(coef_df$estimate < 0)
cat("\nPositive coefficients:", n_pos, "\n")
cat("Negative coefficients:", n_neg, "\n")
cat("Mixed signs indicate balanced model\n")
...
cat("───────────────────────────────────────\n")
cat("Gini:", round(gini_test, 4), " (target ≥ 0.45)\n")
cat("KS:  ", round(ks_test, 4), " (target ≥ 0.25)\n")
cat("AUC: ", round(auc_test, 4), "\n")

# check targets
cat("\nTarget Achievement:\n")
cat("  Gini ≥ 0.45?", ifelse(gini_test >= 0.45, "YES", "NO"), "\n")
cat("  KS ≥ 0.25?", ifelse(ks_test >= 0.25, "YES", "NO"), "\n")
```
...
train_xgb <- train %>% select(-target)
test_xgb <- test %>% select(-target)
...
# extract best iteration and AUC from evaluation log
if (!is.null(model_xgb$evaluation_log) && nrow(model_xgb$evaluation_log) > 0) {
  best_iteration <- nrow(model_xgb$evaluation_log)
  best_auc <- model_xgb$evaluation_log[best_iteration, "test_auc"]
} else {
  best_iteration <- 79  # from training output
  best_auc <- 0.660328
}
...
test_with_pred <- test %>%
  mutate(
    ...
    pred_xgb = pred_prob_xgb[1:nrow(test)]  # align with test set
  )
